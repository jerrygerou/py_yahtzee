...
class Game:
    ...
    def ask_for_category(final_dice):
        next_move = input("How do you want to get points? (ones or twos)")
        if next_move == 'ones':
            return record_ones(final_dice, get_running_points())
        if next_move == 'twos':
            return record_twos(final_dice, get_running_points())
        else:
            pass
